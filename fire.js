...
function updateFireIntensivityPorPixel(currentPixelIndex)
{
...
function CalculateFirePropagation()
{
    for (let column = 0; column < fireWidth; column++)
    {
        for (let row = 0; row < fireHeight; row++)
        {
            ...
            updateFireIntensivityPorPixel(pixelIndex)
        }   
    }
...
function CreateFireSource()
{
    for (let column = 0; column <= fireWidth; column ++)
    {
        const overflowPixerlIndex = fireWidth * fireHeight
        const pixelIndex = (overflowPixerlIndex - fireWidth) + column
